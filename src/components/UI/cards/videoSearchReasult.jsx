      ...
      <div className="flex gap-3">
        <div className="flex justify-end items-end w-[20rem] h-[12rem] z-40">
          ...
          {data?.video?.lengthSeconds &&
            <div className={`${data?.video?.lengthSeconds >= 3600 ? '-ml-[4.2rem]' : '-ml-[3rem]'} mb-[.2rem] bg-[#00ccff] text-gray-800 flex flex-col justify-center px-1 rounded-lg h-fit items-center`}>
              <span className="font-bold text-sm">{toHHMMSS(data?.video?.lengthSeconds)}</span>
            </div>}
        </div>
        ...
        <div className="flex flex-col items-start gap-4">
          <div className="flex flex-col">
            ...
            {data?.video?.badges[0] === "LIVE" ?
              ...
              <span className="text-sm text-left">{data?.video?.stats?.viewers} viewers</span>
              : <div className="flex gap-2">
                <span className="text-sm">{data?.video?.stats?.views} views</span>
                <span className="text-sm">{data?.video?.publishedTimeText}</span>
              </div>}
          </div>
...
export default videoSearchReasult;
